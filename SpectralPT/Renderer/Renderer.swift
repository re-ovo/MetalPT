...
final class Renderer: NSObject, MTKViewDelegate {
    ...
    private var previousDispersion = true
    ...
    func render(to output: MTLTexture, drawable: CAMetalDrawable? = nil) throws -> UInt64? {
        ...
        if size != SIMD2(width, height) {
            accumulation = try context.buffer(width * height * 16, "Persistent XYZ mean")
            size = SIMD2(width, height)
            reset = true
        }
        if previousCamera != model.camera || previousDepth != model.maxDepth
            || previousReset != model.resetToken || previousDispersion != model.dispersion
        {
            reset = true
        }
        ...
        previousDispersion = model.dispersion
        if reset {
            sampleIndex = 0
            generation += 1
        }
