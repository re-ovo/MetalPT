...
- `Renderer/PathTracingPasses.swift`：pass 注册、访问声明和 dispatch；`RenderGraph.swift`：通用图编译与同步。
- `Scene/`：`SceneMesh` 生成几何，`ProceduralScene` 描述内置场景，`SpectralData` 读取光谱数据。`Renderer/BindlessScene.swift` 负责 GPU 上传与加速结构配置。
- `Shaders/`：`Sampling.h`、`Spectrum.h`、`BSDF.h` 提供可复用数学；`Wavefront.metal`、`Display.metal`、`Validation.metal` 分别实现路径追踪、显示和数值验证。

新增 pass 时同时维护 `PathTracingPasses` 的资源访问声明、着色器入口及 `MetalContext` 的 pipeline 注册。共享 MSL 函数放入带包含保护的头文件并声明为 `inline`；kernel 只在一个 `.metal` 编译单元定义。CPU/GPU 数据布局继续统一维护在 `Renderer/Shared.h`。
