...
每帧槽有独立资源池，使用稳定的 `TransientPool.Key` 标识资源，显示名称不参与身份判定；同帧重复租用同一个键会报错。图分配阶段还会检查不同逻辑资源是否意外使用同一分配。buffer 按容量增长并复用，texture 按描述匹配；每帧清除未使用的缓存，只保留当前工作集。活跃工作集超过 256 MiB 时仍保留复用，避免大分辨率下每帧清空并重新分配。资源池不限制活跃内存总量，buffer 容量在持续使用期间保留增长后的大小；在途帧独立持有其使用的分配。
